...
// The curated trophy roster: 60 trophies across three categories (Volume,
// Intensity, Mastery — 20 each), ranked 1..60 by difficulty. Names are catchy but clear
...
fn definitions() -> Vec<Def> {
    vec![
        ...
        def("spend10", "Pocket Change", "Reach $10 of estimated compute", COMMON, VOLUME, 2,
            Box::new(|s| (s.cost / 10.0, s.cost >= 10.0))),
        def("first_million", "First Million", "Process 1,000,000 tokens", COMMON, VOLUME, 6,
            Box::new(|s| (s.processed as f64 / 1_000_000.0, s.processed >= 1_000_000))),
        def("five_m", "5M Club", "Process 5,000,000 tokens", COMMON, VOLUME, 7,
            Box::new(|s| (s.processed as f64 / 5_000_000.0, s.processed >= 5_000_000))),
        def("tens", "10M Club", "Process 10,000,000 tokens", RARE, VOLUME, 10,
            Box::new(|s| (s.processed as f64 / 10_000_000.0, s.processed >= 10_000_000))),
        def("spend100", "First Hundred", "Reach $100 of estimated compute", RARE, VOLUME, 11,
            Box::new(|s| (s.cost / 100.0, s.cost >= 100.0))),
        def("twenty_five_m", "25M Club", "Process 25,000,000 tokens", RARE, VOLUME, 15,
            Box::new(|s| (s.processed as f64 / 25_000_000.0, s.processed >= 25_000_000))),
        def("spend250", "Quarter-Grand", "Reach $250 of estimated compute", RARE, VOLUME, 23,
            Box::new(|s| (s.cost / 250.0, s.cost >= 250.0))),
        def("fifty_m", "50M Club", "Process 50,000,000 tokens", RARE, VOLUME, 24,
            Box::new(|s| (s.processed as f64 / 50_000_000.0, s.processed >= 50_000_000))),
        def("hundred", "100M Club", "Process 100,000,000 tokens", EPIC, VOLUME, 26,
            Box::new(|s| (s.processed as f64 / 100_000_000.0, s.processed >= 100_000_000))),
        def("spend500", "Five Hundred Club", "Reach $500 of estimated compute", EPIC, VOLUME, 27,
            Box::new(|s| (s.cost / 500.0, s.cost >= 500.0))),
        def("spend1k", "Big Spender", "Reach $1,000 of estimated compute", EPIC, VOLUME, 33,
            Box::new(|s| (s.cost / 1000.0, s.cost >= 1000.0))),
        def("archive", "Half-Billion", "Process 500,000,000 tokens", EPIC, VOLUME, 43,
            Box::new(|s| (s.processed as f64 / 500_000_000.0, s.processed >= 500_000_000))),
        def("quarter_b", "Quarter-Billion", "Process 250,000,000 tokens", EPIC, VOLUME, 44,
            Box::new(|s| (s.processed as f64 / 250_000_000.0, s.processed >= 250_000_000))),
        def("high_roller", "High Roller", "Reach $2,500 of estimated compute", EPIC, VOLUME, 45,
            Box::new(|s| (s.cost / 2500.0, s.cost >= 2500.0))),
        def("billion", "Billionaire", "Process 1,000,000,000 tokens", LEGENDARY, VOLUME, 47,
            Box::new(|s| (s.processed as f64 / 1_000_000_000.0, s.processed >= 1_000_000_000))),
        def("spend5k", "Whale", "Reach $5,000 of estimated compute", LEGENDARY, VOLUME, 50,
            Box::new(|s| (s.cost / 5000.0, s.cost >= 5000.0))),
        def("two_half_b", "2.5B Club", "Process 2,500,000,000 tokens", LEGENDARY, VOLUME, 55,
            Box::new(|s| (s.processed as f64 / 2_500_000_000.0, s.processed >= 2_500_000_000))),
        def("five_billion", "5B Club", "Process 5,000,000,000 tokens", LEGENDARY, VOLUME, 56,
            Box::new(|s| (s.processed as f64 / 5_000_000_000.0, s.processed >= 5_000_000_000))),
        def("spend10k", "Kraken", "Reach $10,000 of estimated compute", LEGENDARY, VOLUME, 58,
            Box::new(|s| (s.cost / 10000.0, s.cost >= 10000.0))),
        def("ten_billion", "Ten-Billion Titan", "Process 10,000,000,000 tokens", LEGENDARY, VOLUME, 59,
        ...
        def("streak3", "Three-Peat", "Hold a 3-day activity streak", COMMON, INTENSITY, 3,
            Box::new(|s| (best_streak(s) as f64 / 3.0, best_streak(s) >= 3))),
        def("early_riser", "Early Riser", "Peak activity between 5 and 8 AM (25+ events)", COMMON, INTENSITY, 5,
            Box::new(|s| {
                let p = if (5..8).contains(&s.peak_hour) { (s.peak_events as f64 / 25.0).min(1.0) } else { 0.0 };
                (p, (5..8).contains(&s.peak_hour) && s.peak_events >= 25)
            })),
        def("streak7", "Week Warrior", "Hold a 7-day activity streak", COMMON, INTENSITY, 8,
            Box::new(|s| (best_streak(s) as f64 / 7.0, best_streak(s) >= 7))),
        def("focused2", "In the Zone", "2h+ of focused work in a single day", RARE, INTENSITY, 12,
            Box::new(|s| (s.max_day_focus_sec as f64 / (2.0 * 3600.0), s.max_day_focus_sec >= 2 * 3600))),
        def("nightowl", "Night Owl", "Peak activity before 4 AM (50+ events)", RARE, INTENSITY, 13,
            Box::new(|s| {
                let p = if s.peak_hour < 4 { (s.peak_events as f64 / 50.0).min(1.0) } else { 0.0 };
                (p, s.peak_hour < 4 && s.peak_events >= 50)
            })),
        def("marathon", "Marathoner", "Process 1,000,000 tokens in a single day", RARE, INTENSITY, 14,
            Box::new(|s| (s.max_day_processed as f64 / 1_000_000.0, s.max_day_processed >= 1_000_000))),
        def("big_day5", "Big Day", "Process 5,000,000 tokens in a single day", RARE, INTENSITY, 16,
            Box::new(|s| (s.max_day_processed as f64 / 5_000_000.0, s.max_day_processed >= 5_000_000))),
        def("focused4", "Locked In", "4h+ of focused work in a single day", RARE, INTENSITY, 20,
            Box::new(|s| (s.max_day_focus_sec as f64 / (4.0 * 3600.0), s.max_day_focus_sec >= 4 * 3600))),
        def("deep_work", "Deep Diver", "6h+ of focused work in a single day", RARE, INTENSITY, 21,
            Box::new(|s| (s.max_day_focus_sec as f64 / (6.0 * 3600.0), s.max_day_focus_sec >= 6 * 3600))),
        def("streak14", "Fortnight", "Hold a 14-day activity streak", RARE, INTENSITY, 25,
            Box::new(|s| (best_streak(s) as f64 / 14.0, best_streak(s) >= 14))),
        def("streak21", "Three-Week Run", "Hold a 21-day activity streak", EPIC, INTENSITY, 31,
            Box::new(|s| (best_streak(s) as f64 / 21.0, best_streak(s) >= 21))),
        def("ultra_day", "Heavy Lifter", "Process 10,000,000 tokens in a single day", EPIC, INTENSITY, 34,
            Box::new(|s| (s.max_day_processed as f64 / 10_000_000.0, s.max_day_processed >= 10_000_000))),
        def("deep_work8", "All-Dayer", "8h+ of focused work in a single day", EPIC, INTENSITY, 36,
            Box::new(|s| (s.max_day_focus_sec as f64 / (8.0 * 3600.0), s.max_day_focus_sec >= 8 * 3600))),
        def("streak30", "Iron Streak", "Hold a 30-day activity streak", EPIC, INTENSITY, 38,
            Box::new(|s| (best_streak(s) as f64 / 30.0, best_streak(s) >= 30))),
        def("streak45", "Relentless", "Hold a 45-day activity streak", EPIC, INTENSITY, 40,
            Box::new(|s| (best_streak(s) as f64 / 45.0, best_streak(s) >= 45))),
        def("streak60", "Iron Will", "Hold a 60-day activity streak", EPIC, INTENSITY, 46,
            Box::new(|s| (best_streak(s) as f64 / 60.0, best_streak(s) >= 60))),
        def("colossus", "Powerhouse", "Process 25,000,000 tokens in a single day", LEGENDARY, INTENSITY, 48,
            Box::new(|s| (s.max_day_processed as f64 / 25_000_000.0, s.max_day_processed >= 25_000_000))),
        def("streak100", "Unstoppable", "Hold a 100-day activity streak", LEGENDARY, INTENSITY, 49,
            Box::new(|s| (best_streak(s) as f64 / 100.0, best_streak(s) >= 100))),
        def("ludicrous_day", "Ludicrous Speed", "Process 100,000,000 tokens in a single day", LEGENDARY, INTENSITY, 52,
            Box::new(|s| (s.max_day_processed as f64 / 100_000_000.0, s.max_day_processed >= 100_000_000))),
        def("mega_day", "Juggernaut", "Process 50,000,000 tokens in a single day", LEGENDARY, INTENSITY, 57,
        ...
        def("two_tools", "Double Up", "Run 2 different AI tools", COMMON, MASTERY, 4,
            Box::new(|s| (s.tools_used as f64 / 2.0, s.tools_used >= 2))),
        def("hopper", "Model Hopper", "Use 4 distinct models", COMMON, MASTERY, 9,
            Box::new(|s| (s.models_used as f64 / 4.0, s.models_used >= 4))),
        def("polyglot3", "Triple Threat", "Run 3 different AI tools", RARE, MASTERY, 17,
            Box::new(|s| (s.tools_used as f64 / 3.0, s.tools_used >= 3))),
        def("models6", "Model Maven", "Use 6 distinct models", RARE, MASTERY, 18,
            Box::new(|s| (s.models_used as f64 / 6.0, s.models_used >= 6))),
        def("cache_half", "Cache Apprentice", "Cache reads are 50%+ of billed tokens", RARE, MASTERY, 19,
            Box::new(|s| (cache_ratio(s) / 0.50, cache_ratio(s) >= 0.50))),
        def("active_days_30", "Creature of Habit", "Reach 30 active days", RARE, MASTERY, 22,
            Box::new(|s| (s.active_days as f64 / 30.0, s.active_days >= 30))),
        def("polymath", "Model Master", "Use 8 distinct models", EPIC, MASTERY, 28,
            Box::new(|s| (s.models_used as f64 / 8.0, s.models_used >= 8))),
        def("polyglot5", "Full Stack", "Run 5 different AI tools", EPIC, MASTERY, 29,
            Box::new(|s| (s.tools_used as f64 / 5.0, s.tools_used >= 5))),
        def("all_six_tools", "Whole Ecosystem", "Run 6 different AI tools", EPIC, MASTERY, 30,
            Box::new(|s| (s.tools_used as f64 / 6.0, s.tools_used >= 6))),
        def("active_days_50", "Committed", "Reach 50 active days", EPIC, MASTERY, 32,
            Box::new(|s| (s.active_days as f64 / 50.0, s.active_days >= 50))),
        def("around_clock", "Around the Clock", "Be active in 20+ different hours of the day", EPIC, MASTERY, 35,
            Box::new(|s| (s.active_hours as f64 / 20.0, s.active_hours >= 20))),
        def("around_clock_24", "Round the Clock", "Be active in all 24 hours of the day", EPIC, MASTERY, 37,
            Box::new(|s| (s.active_hours as f64 / 24.0, s.active_hours >= 24))),
        def("cache_adept", "Cache Adept", "Cache reads are 80%+ of billed tokens", EPIC, MASTERY, 39,
            Box::new(|s| (cache_ratio(s) / 0.80, cache_ratio(s) >= 0.80))),
        def("cache_master", "Cache King", "Cache reads are 95%+ of billed tokens", EPIC, MASTERY, 41,
            Box::new(|s| (cache_ratio(s) / 0.95, cache_ratio(s) >= 0.95))),
        def("models10", "Model Maestro", "Use 10 distinct models", EPIC, MASTERY, 42,
            Box::new(|s| (s.models_used as f64 / 10.0, s.models_used >= 10))),
        def("omnivore", "Model Legend", "Use 12 distinct models", LEGENDARY, MASTERY, 51,
            Box::new(|s| (s.models_used as f64 / 12.0, s.models_used >= 12))),
        def("models_16", "Model Olympian", "Use 16 distinct models", LEGENDARY, MASTERY, 53,
            Box::new(|s| (s.models_used as f64 / 16.0, s.models_used >= 16))),
        def("veteran", "Veteran", "Reach 100 active days", LEGENDARY, MASTERY, 54,
            Box::new(|s| (s.active_days as f64 / 100.0, s.active_days >= 100))),
        def("lifer", "Lifer", "Reach 200 active days", LEGENDARY, MASTERY, 60,
...
mod tests {
    ...
    fn roster_has_unique_ordered_ranks() {
        ...
        ranks.sort_unstable();
        assert_eq!(ranks, (1..=60).collect::<Vec<u32>>());
        // Every category has exactly 20 trophies.
        for cat in [VOLUME, INTENSITY, MASTERY] {
            assert_eq!(list.iter().filter(|a| a.category == cat).count(), 20, "{cat}");
        }
